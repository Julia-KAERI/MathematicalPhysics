...
### 삼각함수의 적분 {#sec-CF_trigonometric_integral}

아래와 같은 꼴의 적분을 생각하자.

$$
I = \int_0^{2\pi} f(\sin \theta,\, \cos\theta)\, d\theta.
$$ {#eq-CF_trigonometric_form_1}

여기서 $f$ 는 모든 $\theta$ 에 대해 유한하며 $\sin \theta,\ \cos\theta$ 에 대해 meromorphic 이라고 하자. 즉 단일값 함수이다. 변수를 $z=e^{i\theta},\, dz=ie^{i\theta}$ 로 바꾼다면  @eq-CF_trigonometric_form_1 는 복소평면상의 단위원 $C_1$ 에 대한 경로적분으로 아래와 같이 바꿀 수 있다.

$$
I = -i \oint_{C_1} f\left(\dfrac{z-z^{-1}}{2i},\, \dfrac{z+z^{-1}}{2} \right) \, \dfrac{dz}{z}.
$${#eq-CF_trigonometric_form_2}

$f$ 가 merohorphi 이므로 유수정리를 이용할 수 있으며 $\{z_j\}$ 가 단위원 내에서의 단순 고립점이라고 하면

$$
I = (-i) 2\pi i \sum_j \text{Res}(f,\, z_i)
$${#eq-CF_trigonometric_form_3}
...
### $-\infty$ 에서 $\infty$ 로의 적분 {#sec-CF_integral_infinite}

아래와 같은 형태의 적분을 구해야 한다고 하자.

$$
I = \int_{-\infty}^\infty f(x)\, dx
$$ {#eq-CF_infinity_integral}
...
보통 @eq-CF_infinity_integral 형태의 적분을 계산 할 때 위의 두 조건을 만족하는 복소평면의 위쪽이나 아래쪽 반평면을 선택하게 된다. 우선 위쪽 반평면을 생각하기로 하자. 그리고 아래 @fig-CF_infinite_integral 와 같은 contour 를 생각한다. $z_1,\,z_2,\ldots$ 는 극점이며 contour $C$ 는 $C_R+C_I$ 라고 하자. 

![큰 반원으로 둘러쌓인 위쪽 반평면에서의 contour](figures/fig-complex_infinite_integral.png){#fig-CF_infinite_integral width=400}
...
### 복소 지수함수가 포함된 적분 {#sec-CF_integrals_with_complex_exponent}

실수 $a$ 에 대해 아래와 같은 형태의 적분을 생각하자.
$$
I = \int_{-\infty}^\infty f(x) e^{iax}\, dx
$$ {#eq-CF_integral_including_exponential_1}
...
우리는 @fig-CF_infinite_integral 에서의 위쪽 반평면에서의 $C_R,\,C_I$ 로 이루어진 contour 를 생각하자. $C_R$ 에서의 적분을 $I_R$ 이라고 하고 $z=Re^{i\theta}$ 로 잡자.
...
$$ {#eq-CF_integral_including_exponential_2}
...
$$ {#eq-CF_integral_including_exponential_3}
...
$$ {#eq-CF_integral_including_exponential_4}

이다. 

</br><div class="border" style="background-color:#F2F4F4  ;padding:5px;">

::: {#exm-CF_integral_including_exponential_1}
...
::: {#exm-CF_integral_including_exponential_2}
...
이다. 이제 @fig-CF_cauchy_principal_value_2 에서의 경로대로 적분을 구해보자. 
...
::: {#exm-CF_integral_wihtout_symmetry_1}
...
이다. 우리는 $r\to \infty$ 극한에서 $C_R$ 경로에서의 적분이 $0$ 이라는 것을 안다(@sec-CF_integral_infinite). 또한 contour $C_1+C_R+C_2$ 내의 극점이 $e^{\pi/3}$ 뿐이므로
...
::: {#exm-CF_integration_avoiding_branch_points_1}
...
함수 $f(x) = \dfrac{\ln x}{x^3+1}$ 에 대해 $x=0$ 은 특이점이다. 또한 복소평면에서 branch point 이므로 contour 를 잡을 때 $z=0$ 을 피해야 하며 $0$ 에서 $z=\infty$ 로 향한 branch cut 을 잡고 그것도 피해야 한다. 여기서는 아래와 같은 contour 를 잡기로 하자. @fig-complex_integral_without_symmetry 와 유사하다. 또한 $f(x)$ 는 @exm-CF_integral_wihtout_symmetry_1 과 같은 극점을 가진다. 
...
을 생각하자. $C_R$ 에 대한 적분은 @sec-CF_integral_infinite 의 조건에 따라 $0$ 이 된다. $C_1$ 에서의 적분은 $I_1$ 이라고 하자. 즉
...
이며 @exm-CF_integral_wihtout_symmetry_1 에서 $\int_0^\infty \dfrac{dx}{x^3+1} = \dfrac{2\pi}{3\sqrt{3}}$ 임을 보였다. 또한 $C$ 내부의 극점은 $z=e^{i\theta_0}$ 뿐이므로
...
### Branch Cuts 를 이용한 적분 {#sec-CF_exploiting_branch_cuts}
...
::: {#exm-CF_exploiting_branch_cuts_1}
...
그리고 아래 @fig-CF_exploiting_branch_cuts 의 contour 에서의 복소평면에서의 적분
...
![Contour](figures/fig-complex_exploiting_branch_cuts.png){#fig-CF_exploiting_branch_cuts width=350}
...
::: {#exm-CF_exploiting_branch_cuts_2}

@exm-CF_integral_wihtout_symmetry_1 에서 했던 아래의 적분을 다시 계산해보자.

$$
I = \int_{0}^\infty \dfrac{dx}{x^3+1}.
$$

이 적분의 계산을 @fig-CF_exploiting_branch_cuts 의 contour 에서 
...
### 주기함수의 적분 {#sec-CF_exploiting_periodicity}
...
::: {#exm-CF_exploiting_periodicity_1}
...
$\sinh(iy) = i \sin(y)$ 임을 안다. 이제 @fig-CF_exploiting_periodicity_1 의 contour 에서의 적분
...
위로부터 @fig-CF_exploiting_periodicity_1 의 contour 에서의 극점은 $z=i\pi$ 뿐임을 안다. 그렇다면, 
...
![Contour](figures/fig-complex_exploiting_periodicity_1.png){#fig-CF_exploiting_periodicity_1 width=400}
...
@exm-CF_trigonomeetric_1 의 적분을 일반화 한 아래의 식이 성립함을 보여라.
...
@exm-CF_exploiting_branch_cuts_1 에서 $R\to \infty$ 일 때의 $C_R$ 경로에서의 적분과 $\varepsilon \to 0$ 에서의 $C_\varepsilon$ 경로에서의 적분값이 모두 $0$ 임을 보여라.
...
$0<p<1$ 에 대해 $f(z) = \dfrac{z^p}{z^2+1}$ 이라고 하자. @sec-CF_integral_infinite 의 두가지 조건을 $f$ 가 만족하기 때문에 $C_R$ 경로에서의 적분은 $R\to \infty$ 극한에서 $0$ 이다. $C_\varepsilon$ 경로에서의 적문을 생각하자. $z=\varepsilon e^{i\theta},\, dz=i\varepsilon e^{i\theta}\,d\theta$ 이므로
...
::: {.solution}
 ...
 @exm-CF_integral_including_exponential_2 로부터 $\displaystyle \int_{-\infty}^\infty \dfrac{\sin t}{t} \, dt = \pi$ 임을 안다. 앞의 적분에서 $s>0$ 에 대해 $t=sx$ 로 환하면 $\displaystyle \int_{-\infty}^\infty \dfrac{\sin (sx)}{x}\, dx= \pi$ 이다. 
...
우선 @fig-CF_infinite_integral 의 경로대로 위쪽 반평면에서 $\oint_C f(z)\,dz$ 를 계산해 보자. $C_R$ 에 대한 경로적분은 $0$ 이며 경로상의 고립점은 $z=i$ 이므로
...
($a$) $f(z) = \dfrac{e^{iz}}{z^2+a^2}$ 이라고 하면 $\displaystyle \dfrac{\cos x}{x^2+a^2} = \dfrac{f(x)+f(-x)}{2}$ 이다. 위쪽 반평면에 대해 @fig-CF_infinite_integral 경로의 적분을 생각하자. $C_R$ 경로 적분값은 $0$ 이며, 경로 내부의 고립점은 $ia$ 이므로
...
($b$) $f(z) = \dfrac{ze^{iz}}{z^2+a^2}$ 라고 하면 $\dfrac{x\sin x}{x^2+a^2} = \dfrac{f(z)+f(-z)}{2i}$ 이다. @fig-CF_infinite_integral 의 경로를 생각하면 $C_R$ 경로의 적분은 $R\to \infty$ 극한에서 $0$ 이며 contour 내의 고립점은 $ia$ 이다. 따라서, 
...
![Contour](figures/fig-complex_contour_Arfken_11_8_13.png){#fig-CF_contour_Arfket_11_8_13 width=350}
:::
...
![Contour](figures/fig-complex_contour_Arfken_11_8_14.png){#fig-CF_contour_Arfken_11_8_14.png}
...
이다. @fig-CF_infinite_integral 복소평면위 위쪽 반평면에서 정의된 반원형 contour 를 생각하자. $f(z)$ 은 @sec-CF_integral_infinite 에서의 조건을 만족하며, 따라서 $R\to \infty$ 극한에서 $\int_{C_R}f(z)\,dz = 0$ 이다. 즉
...
$f(z) = \dfrac{z^2}{z^4+1}$ 의 극점은 $z_1=\dfrac{1+i}{\sqrt{2}}$, $z_2=\dfrac{-1+i}{\sqrt{2}}$, $z_3= \dfrac{-1-i}{\sqrt{2}}$, $z_4= \dfrac{1-i}{\sqrt{2}}$ 이다. @fig-CF_infinite_integral 복소평면위 위쪽 반평면에서 정의된 반원형 contour 를 생각하자. $f(z)$ 은 @sec-CF_integral_infinite 에서의 조건을 만족하며, 따라서 $R\to \infty$ 극한에서 $\int_{C_R}f(z)\,dz = 0$ 이다. 네개의 극점중 위쪽 반평면에 위치하는 것은 $z_1,\,z_2$ 이므로
...
$I = \displaystyle \int_{0}^\infty \dfrac{x^p \ln x}{x^2+1}\,dx$ 라고 하고 @fig-CF_exploiting_branch_cuts 의 contour 를 생각하자. $f(z) = \dfrac{z^p\ln z}{z^2+1}$ 은 $z_1=e^{\pi i/2},\, z_2=e^{i3\pi/2}$ 에서 극점을 가진다. 따라서, 
...
이다. $C_R$ 경로 적분에서 $\lim_{x\to \infty}xf(x) = 0$ 이므로 $C_R$ 경로상의 적분의 기여는 $0$ 이다. $C_2$ 경로의 적분은 $z=xe^{2i\pi}$ 에 대한 적분이며 @exm-CF_exploiting_branch_cuts_1 의 결과를 사용하면
...
![Contour](figures/fig-complex_contour_Arfken_11_8_18.png){#fig-CF_contour_Arfken_11_8_18 width=400}
