...
$z=0$ 이 branch point 이며 양의 실수축을 branch cut 으로 잡닸다. $z^p$ 는 다가함수 이지만 코시 주요값으로 branch cuts 의 바로 윗부분 ($C_1$) 으로 정했다. 짐작하듯이 $\varepsilon \to 0$ 극한과 $R \to \infty$ 극한을 취할 것이다. $C_R$ 에 대한 적분은 이미 알고 있듯이 $R\to \infty$ 적분에서 $0$ 이며, $C_\varepsilon$ 적분도 $\varepsilon\to 0$ 극한에서 $0$ 이다.  $C_1$ 적분이 $I$ 이며, $C_2$ 적분 $I_2$ 는 $z=re^{i2\pi}$ 에 대해
...
### 주기를 이용한 적분 {#sec-exploiting_periodicity}
