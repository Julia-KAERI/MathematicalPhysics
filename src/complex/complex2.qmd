...
### 특이점 {#sec-CF_singulariity}

::: {.callout-note appearance="minimal"}

::: {#def-CF_isolated_point}
...
### 분지점(Branch point) {#sec-CF_branch_points}
...
::: {#exm-CF_z_12_on_closed_loop}

![원점 주위의 $z^{1/2}$ 값을 계산하기 위한 경로](figures/fig-complex_branch_1.png){#fig-CF_branch_1 width=300}
...
::: {#exm-CF_z_12_on_closed_loop_2}

![원점 주위가 아닌, $z^{1/2}$ 값을 계산하기 위한 경로](figures/fig-complex_branch_2.png){#fig-CF_branch_2 width=350}

$z=2$ 을 중심으로 반지름이 1 인 원 경로를 생각하자(@fig-CF_branch_2). $A$ 점, 즉 $z=3$ 에 대해 $f(3) = \pm \sqrt{3}$ 값을 가질 수 있다. $z=2+e^{i\theta}$ 의 경로를 생각하면 $\text{Re}(z) > 0$ 이며 따라서 $ABCDA$ 순으로 순회하여 다시 $A'=A$ 로 갔을 때의 값은 역시 $\sqrt{3}$ 임을 알 수 있다. 즉 @exm-CF_z_12_on_closed_loop 의 경우와 달리 closed loop 를 순회하고 다시 돌아왔을 때 그 함수값이 바뀌지 않는다.

:::

</div></br>

@exm-CF_z_12_on_closed_loop 와 @exm-CF_z_12_on_closed_loop_2  의 차이는 경로 내부에 특이점 $z$ 를 포함하는지 여부이다. 여기서 $z=0$ 이 왜 특별한가? $f(z) = z^{1/2}$ 는 $z=0$ 에서 미분이 정의되지 않은 특이점이며 이 특이점 주변의 경로에 따라 함수값을 계산할 경우 여려 값을 가질 수 있다. 이런 점을 **branch point** 라고 하며, branch point 의 계수(order)는 함수가 원래의 값으로 돌아오기 위해 거쳐야 하는 경로의 수를 말한다. @exm-CF_z_12_on_closed_loop 의 경우 $z=0$ 은 $f(z) = z^{1/2}$ 에 대해 계수가 $2$ 인 branch point 이다. 
...
::: {#exm-CF_branches_of_log_Z}

$f(z) = \ln z$ 를 생각하자. 앞서 @sec-CF_exponential_and_log 에서 보았듯이 $\ln z$ 는 다가함수이며, 극좌표 표현에서
...
이므로 $z\to \infty$ 에서 단순 극점을 가진다. 이제 적절한 branch cut 을 구성하여 $f(z)$ 를 단일값 함수로 제한해보자. 방법은 많지만 @fig-CF_branch_3 에 나온 branch cut 을 이용해 보기로 하자. 

![가능한 브랜치중의 하나](figures/fig-complex_branch_3.png){#fig-CF_branch_3  width=300}
...
이다. 이제 @fig-CF_branch_4 의 closed path 를 생각하자. $A$ 로 부터 시작하여 $B,C,D,E,F$ 를 거쳐 다시 $A$ 로 돌아오는 경로(이때의 $A$ 를 $A'$ 이라고 표기하고 구분하도록 하자)를 생각한다. $A$ 에 대해 $\theta = \varphi = 0$ 으로 놓으면 $f(2) = \sqrt{3}$ 이 된다. 

![Branch cut 주위의 경로](figures/fig-complex_branch_4.png){#fig-CF_branch_4 width=450}
...
| $F$ | $2\pi$ |$\pi$ | $3/2\pi$ |
| $A'$ | $2\pi$ |$2\pi$ | $2\pi$ |
: Phase angles {#tbl-CF_phase_angle}
...
### 해석적 연속 {#sec-CF_analytic_continuation}
...
이다. 이 때 로랑 전개에의 $(z-z_0)^{-1}$ 의 계수를 **유수 (留數, residue)** 라고 하고 $\text{Res}(f,\,x_0)$ 라고 표기한다. 이제 고립점 $z_1,\,z_2,\ldots,\,z_n$ 를 가진 함수 $f(z)$ 에 대해 닫힌 contour 에서 적분한다고 생각하자. 우리는 이 contour 를 @fig-CF_residue_1 와 같이 변형하여 적분할 수 있으며, 
...
![고립점을 제외한 경로 잡기](figures/fig-complex_residue_1.png){#fig-CF_residue_1 width=300}
...
($4$) $f_4(z)=\dfrac{z}{\sin^2 z }$ 의 $z=\pi$ 에서의 residue 를 계산해 보자. 극점의 계수가 2 이므로 residue 는 @eq-CF_residual_2 에 따라
...
### 코시 주요값 {#sec-CF_cauchy_principal_value}
...
가 된다. @fig-CF_cauchy_principal_value 에서 볼 수 있듯이 $(-\delta,\, \delta)$ 구간의 적분값은 서로 상쇄되어 $0$ 이된다. 그런데 이 과정은 적분에서의 일반적인 과정과는 다른데 적분의 일반적인 과정 혹은 정의에 의거하면 왼쪽과 오른쪽의 $0$ 에 접근하는 극한이 독립적이어야 한다. 즉, 
...
![$1/z$ 적분에 있어서의 코시 주요값 소거](figures/fig-complex_cauchy_principal_value_1.png){#fig-CF_cauchy_principal_value width=400}
...
::: {#exm-CF_cauchy_principal_value_1}
...
앞서 언급했듯이 코시 주요값은 적분으로 정의되긴 하지만 적분의 일반적인 정의를 벗어난다. 그렇다면 이게 무슨 의미가 있을까? $x_0$ 를 제외하고 실수축에서 적분하는 대신 @fig-CF_cauchy_principal_value_2 와 같은 복소평면상에서의 적분을 생각하자. 고립특이점 $x_0$ 근처에서 반지름이 $r$ 인 작은 시계방향의 반원 contour $C_1$ 을 생각하자. 여기서는 $x_0$ 가 단순 극점(simple pole) 이라고 하자.(일반적인 계수 $n$ 의 극점에 대해서는 이후에 다룬다.)

![코시 주요값 적분을 포함하는 contour](figures/fig-complex_cauchy_principal_value_2.png){#fig-CF_cauchy_principal_value_2 width=400}
...
이제 @fig-CF_cauchy_principal_value_2 의 전체 경로 구간 적분을 생각하자. $\{z_j\}$ 를 그림의 contour 내부에 있는 고립 특이점의 집합이라고 할 때 유수정리에 따라

$$
PV\int f(z)\,dz + I_{C_1} + \int_{C_2} f(z)\, dz = 2\pi i \sum_{j} \text{Res}(f,\,z_j)
$$ {#eq-CF_cahchy_principal_value_2}
...
$$ {#eq-CF_cahchy_principal_value_3}

을 얻는다. $2 \pi i \text{Res}(f,\,z_0) = 2\pi i a_{-1}$ 이므로 두 식은 기본적으로 같은 식이다. $C_2$ 에 대한 적분은 $R\to \infty$ 극한에서 $0$ 에 되는데 이는 @sec-CF_integrals_with_complex_exponent 에서 다루며 위 적분값은 @exm-CF_integral_including_exponential_2 에서 구한다. 
...
앞서 @def-CF_meromorphic 에서 정의했듯이 고립특이점을 제외하고 해석적인 함수를 meromorphic 이라고 한다. 미타그(Mitagg) 와 레플레르(Leffler) 는 해석적인 점 주위로 전개하거나(테일러 전개), 특정 고립특이점 주위로 전개하는(로랑 전개) 것 이외에 다수의 고립특이점에 대해 전개할 수 있다는 것을 보였다. 이를 미타그-레플레르 정리라고 한다. 


<div class="border" style="background-color:#FFF0F5 ;padding:5px;">

::: {#thm-CF_mittag-leffler}
...
$b_n = \text{Res}(f,\,z_n)$ 라고 하자. @eq-CF_mittag_leffler_1 의 $\sum$ 부분은 $\dfrac{zb_n}{z_n (z_n-z)}$ 이다. 이제 $z_1,\ldots,\,z_n$ 만을 포함하는 반지름 $R_n$ 인 원형 contour $C_n$ 를 생각하고
...
\end{aligned}
$$ {#eq-CF_mittag_leffler_4}


</br>

### 극점과 영점의 갯수 {#sec-CF_number_of_zeros_and_poles}
...
$1<|z|<3$ 사이에 $F(z) = z^3-2z+11$ 의 영점이 몇개인지 확인해보자. @eq-CF_poles_and_zeros 을 $|z|=1$ 인 원과 $|z|=3$ 인 원에 대한 contour 에서 사용하는 방법은 다소 복잡하므로 루셰 정리를 사용하기로 하자. $g(z) = z^3-2z,\, f(z) = 11$ 로 잡으면 $F(z) = f(z)+g(z)$ 이며 $|z|=1$ 에서 $|f(z)|>|g(z)|$ 라는 것을 쉽게 보일 수 있다. $f(z)$ 는 영점이 없으므로 $g(z)$ 도 영접이 없다.
...
$$ {#eq-CF_product_expansion_of_entire_function_1}
...
\end{aligned}
$${#eq-CF_product_expansion_of_entire_function_2}
...
이다. $\varepsilon \to 0$ 극한에서 @exm-CF_mittag_leffler_2 를 참고하면
...
@eq-CF_product_expansion_of_entire_function_1 는 @eq-CF_product_expansion_of_entire_function_2 의 $\sin z$ 식으로 직접적으로 유도 될 수 없다는 것을 설명하고 $\sin z/z$ 를 전개하여 얻을 수 있다는 것을 보여라.
...
우선 $\sin 0 = 0$ 이므로 @eq-CF_product_expansion_of_entire_function_1 의 우변의 맨 앞의 $f(0) \exp [zf'(0)/f(0) ]$ 를 제대로 계산 할 수 없다. $g(z) = \sin z/z$ 라고 하면 $\lim_{z\to 0} g(z) = 1$ 이며, 
...
$\sec z$ 와 $\csc z$ 에 대한 곱전개 @eq-CF_mittag_leffler_4 유도하라.
...
@sec-CF_integrals_with_complex_exponent 의 [복소 지수함수가 포함된 적분](complex3.qmd#sec-CF_integrals_with_complex_exponent) 를 이용한다.

($a$) 아래 그림 @fig-CF_cauchy_principal_value_3 와 같은 경로를 생각하자. 

![단위 계단 함수의 복소 적분 표현 1](figures/fig-complex_cauchy_principal_value_3.png){#fig-CF_cauchy_principal_value_3 width=600}
...
($b$) 의 경우는 @fig-CF_cauchy_principal_value_2 의 경로를 생각하자. $s>0$ 일 때 
