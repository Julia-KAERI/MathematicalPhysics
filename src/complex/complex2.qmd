...
연습문제 @Arfken7_11_7_8 에서 다음 극점 전개를 보일것이다.
...
$$
\begin{aligned}
PV \int_{-\infty}^\infty \dfrac{e^{ixs}}{x-i\varepsilon}\, dx &= \int_{-\infty}^{-\delta} \dfrac{e^{ixs}}{x-i\varepsilon} \,dx +  \int_{\delta}^{\infty} \dfrac{e^{ixs}}{x-i\varepsilon} \, dx \\
&= \int_{\delta}^\infty \left(-\dfrac{e^{-ixs}}{x+i\varepsilon} + \dfrac{e^{ixs}}{x-i\varepsilon}\right) \, dx \\
&\int_{\delta}^\infty  
\end{aligned}
$$
