...
주어진 군 $G$ 와 집합 $S$ 에 대해 $\pi \in \text{Hom}(G,\, \text{Sym}(S))$ 를 $S$ 에 대한 $G$ 의 **작용 (action)** 이라고 한다.
...
다음은 준동형 사상과 대칭군의 성질에 의해 쉽게 보일 수 있다.

<div class="border" style="background-color:#FFF0F5 ;padding:5px;">

::: {#thm-Group_basic_properties_of_group_action}

$g\in G$ 에 대해 $\pi(g)$ 를 $\pi_g$ 로 표기하자. 이 때 다음이 성립한다.

&emsp;($1$) $\pi_{1_G}$ 는 항등함수이다.

&emsp;($2$) $x,\,y\in G$ 에 대해 $\pi_x\circ\pi_y = \pi_{xy}$ 이다.
...
정해진 $x\in G$ 에 대해 $\pi_x(g)=xg$ 는 $G$ 에 대한 $G$ 의 작용이다. 
...
::: {#def-Group_orbint_of_action_of_group}

군 $G$ 가 집합 $S$ 에 대한 작용일 때 $G$ 의 **궤도 (orbint)**
