...
6. $g\in G$ 에 대해 $\{g^k:k\in \Z\}=G$ 일 때 $G$ 를 **순환군 (cyclic group)** 이라고 하고 $G=\langle g \rangle$ 이라고 표기한다. 이 때 $G$ 는 $g$ 에 의해 생성된다고 한다.
   
7. $H \subset G$ 이며 $H$ 가 $G$ 와 같은 연산에 대해 군일 때 $H$ 를 $G$ 의 **부분군(subgroup)** 이라고 하고 $H \le G$ 라고 표기한다.
...
$f\in \text{Sym}(X)$ 에서 $a,\,b\in X$ 에 대해 $f(a)=b,\, f(b)=a$ 이며 $x\ne a,\,b$ 일 때 $f(x)=x$ 인 경우 $f$ 를 **호환(transposition)** 이라고 한다. 모든 $\text{Sym}(X)$ 의 원소는 호환의 곱으로 표현 할 수 있다. $g\in \text{Sym}(X)$ 에 대해 호환의 곱은 유일하지 않고 곱해지는 호환의 갯수도 유일하지 않지만 호환의 갯수가 짝수인지 홀수인지는 불변이다. 짝수일 때를 짝순열, 홀수일 때는 홀순열이라고 하며 짝순열의 집합은 $\text{Sym}(X)$ 의 부분군으로 **교대군(alternating group)** 이라고 하며 $A_X$ 로 표현한다.
...
### 군의 부분집합의 연산 {#sec-Group_multplication_of_subset_of_group}
...
#### $D_3$ 군의 conjugace class

@exm-Group_d3_multiplication_table 의 Caylay table 을 이용하여 $D_3$ 의 conjugace class 를 구해보자. 우선 $\{E\}$ 는 conjugace class 이다. $A,\,B,\,C$ 는 각각 스스로의 역원이며 $D,\,F$ 는 서로간의 역원이다. $ABA^{-1}=ABA=C$ 이므로 $B\sim C$ 이다. $BAB^{-1}=A$ 이므로 $A\sim C$ 이다. 이것을 반복해보면 $\{A,\,B,\,C\}$ 가 conjugacy class 임을 알 수 있다. 또한 $AFA^{-1}=D$ 이므로 $F\sim D$ 임을 알 수 있다. 즉 $D_3$ 군에는 아래와 같은 $3$ 개의 conjugace class 가 존재한다.
...
$N \le G$ 이며 각각의 $g\in G$ 에 대해 $gNg^{-1}=N$ 이면 $N$ 을 $G$ 의 **정규부분군(normal subgroup)**,**불변 부분군(invariant subgroup)** 혹은 **normal devisor** 라고 하고 $N \trianglelefteq G$ 라고 표기한다.
...
::: {#prp-Group_left_and_right_cosets_of_normal_subgroup}

군 $G$ 의 부분군 $N$ 에 대해 다음은 동치이다.

&emsp;($1$) $N\trianglelefteq G$ 이다.

&emsp;($2$) 임의의 $g\in G$ 에 대해 $gN = Ng$ 이다.
...
($1 \implies 2$) $x\in gN$ 이면 어떤 $n\in N$ 에 대해 $x=gn$ 이며 $x=gn=\left(gng^{-1}\right)g \in Ng$ 이므로 $gn\subset Ng$ 이다. 같은 방법으로 $Ng\subset gN$ 임을 보일 수 있으며 따라서 $gN=Ng$ 이다.

($2\implies 1$) $gNg^{-1}= N$ 이다.
...
위의 몫군에서 정규부분군 $N$ 은 나눗셈의 제수(divisor) 역할을 수행한다고 볼 수 있다. 이런 이유로 정규부분군을 **normal divisor** 라고 부르기도 한다.

</br>

### Class 곱 {#sec-Group_class_multiplication}

$G$ 의 conjugate class 의 집합 $\mathcal{K}_G=\{K_1,\ldots,\,K_m\}$ 를 생각하자. $g\in G$ 에 대해
...
이다. 이것은 $K_iK_j$ 가 conjugate class 일 경우 뿐만 아니라 conjugatcy class 의 합집합 일 때도 성립하며, 실제로 그러하다. $K_i,\,K_j$ 의 곱이 두개 이상의 conjugace class 의 합집합이며, 어떤 경우 같은 원소가 여러번 등장하기도 한다. 이 때 합집합과 등장 횟수를 모두 포함하는 다음과 같은 식을 정의 할 수 있다.
...
우리는 3차원 이상에서의 회전이 서로 commute 하지 않는다는 것을 안다. @exm-Group_d3_multiplication_table 에서 $AB=F$, $BA=D$ 임을 확인하라. 무한소 회전 $R \approx I+A$ 와 임의의 회전 $R'$ 에 대해 $A^2$ 이상의 항을 무시하면
...
($c$) 전체 군의 가능한 subgroup 을 나열하라. 이 가운데 불변부분은 무엇인가?
...
($1$) $G$ 가 아벨군임을 가정하자. $\phi(x)\phi(y) = x^{-1}y^{-1}=y^{-1}x^{-1}= (xy)^{-1}=\phi(xy)$ 이므로 $\phi$ 는 동형사상이다.

($2$) $\phi$ 가 동형사상임을 가정하자. 임의의 $x,\,y\in G$ 에 대해 
$$
