...
### 무한소 회전 {#sec-Group_infinitesimal_rotation}
...
여기서 $I$ 는 항등행렬이며 $A$ 가 무한소 회전에 의한 변화를 기술하는 행렬이 된다. 위 식에서 $\theta$ 에 대한 2차항 이상의 항은 무시한다. $R$ 은 $SO(2)$ 연산자이므로 $R^T R=I$ 이어야 한다. $R=I+ A$ 에 대해 $R^T = I+A^T$ 이며, $\theta^2$ 이상을 무시하면

$$
R^T R \approx I+ A^T + A + A^TA \approx I+ (A^T+A)
$$

이다. $R^T R=I$ 조건에 의해 $A^T=-A$ 이어야 하는데 (이런 형태의 행렬을 **skew-symmetric** 이라고 한다) $\R^{2\times 2}$ 에는 이것을 만족하는 행렬은 

$$
\mathcal{J} = \begin{bmatrix} 0 & 1 \\ -1 & 0 \end{bmatrix}
$$ {#eq-Group_matrix_for_infinitesimal_rotation_J_for_2d}

와 $-\mathcal{J}$ 뿐이다. 이제 무한소 회전을 나타내는 행렬 $R$ 은 다음과 같이 쓸 수 있다.
...
미적분학의 테일러 정리로부터 우리는 $f$ 의 모든 $n$ 차 도함수를 안다면 $f$ 를 다항식으로 구성할 수 있다는 것을 안다. 반면 여기서는 $SO(2)$ 군의 곱 연산의 구조때문에 $R(\theta)$ 의 첫번째 미분, 즉 $\mathcal{J}$ 만으로 $R(\theta)$ 를 구성 할 수 있다는 것을 리 대수를 창안한 리가 발견하였다. 
