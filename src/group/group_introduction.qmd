...
는 Viergrouppe (@exm-Group_viergrouppe) 와 동형이며 $A_4$ 의 정규부분군이다. 또한 $A_4$ 의 켤레류는 다음과 같다.

$$
\begin{aligned}
&\{I\}, \\[0.3em]
&\{(12)(34),\, (13)(24),\, (14)(23)\}, \\[0.3em]
&\{(123),\, (142),\, (134),\, (243)\}, \\[0.3em]
&\{(132),\, (124),\, (143),\, (234)\}.
\end{aligned}
$$
...
이로 $K_iK_j$ 는 켤레 작용에 대해 닫혀 있다. 즉 $K_iK_j$ 는 어떤 켤레류들의 합집합이다. 여기서 우리는 중요한 정리를 언급하고자 한다. 이 명제는 동일한 명제인 @prp-GroupAux_frobenius_algebra_3 의 증명을 참고하라.

<div class="border" style="background-color:#FFF0F5 ;padding:5px;">

::: {#thm-Group_product_of_two_conjugacy_class}

$K_i,\,K_j$ 가 유한군 $G$ 의 켤레류일 때 $K_iK_j$ 는 컬레류의 합집합이며 반복 횟수를 포함한다면 
...
## 군의 작용 {#sec-Group_action_of_group0}

### 군의 작용 {#sec-Group_action_of_group}

::: {.callout-note icon="false"}

#### **군의 작용**

::: {#def-Group_action_of_group}


주어진 군 $G$ 와 집합 $S$ 에 대해 $\pi : G \times S \to S$ 가 다음을 만족할 때 $\pi$ 를 $S$ 에 대한 $G$ 의 작용이라고 하고 $g\in G,\, s\in S$ 에 대해 $\pi_g (s)$ 라고 표기한다.

&emsp;($1$) 임의의 $x,\,y\in G,\, s\in S$ 에 대해 $\pi_{xy}(s) = \pi_x(\pi_y(s))$ 이다.

&emsp;($2$) 임의의 $s\in S$ 에 대해 $\pi_{1_G}(s)=s$ 이다.
:::
:::
...
::: {#thm-Group_equivalent_definition_of_group_action}

$\phi\in \text{Hom}(G,\,\text{Sym}(S))$ 은 군의 작용이다. 또한 $G$ 에서 $S$ 로의 군의 작용은 $\text{Hom}(G,\,\text{Sym}(S))$ 에 속한다. 즉 $\phi\in \text{Hom}(G,\,\text{Sym}(S))$ 는 @def-Group_action_of_group 와 동등한 정의이다.
...
준 동형사상의 정의에 의해 $\phi$ 는 군의 작용이다. 임의의 $g\in G$ 에 대해 $\pi_g(s_1)=\pi_g(s_2)$ 이면 $s_1=s_2$ 이므로 $\pi$ 는 단사이며 따라서 $\pi\in \text{Hom}(G,\,\text{Sym}(S))$ 이다. 즉 두 정의는 동등하다. $\square$

:::

</br><div class="border" style="background-color:#FFF0F5 ;padding:5px;">

::: {#cor-Group_action_is_injective}

$\pi$ 가 군 $G$ 의 $S$ 로의 작용일 때 $\pi_g : S \to S$ 는 단사이다.
:::
...
::: {#exm-Group_group_action_in_LN}

#### 일반선형군에서의 군의 작용
</br>

($1$) $A\in \text{GL}(n)$ 는 $\R^n \mapsto \R^n$ 전단사 함수 임이 알려져 있다. 또한 $v\in \R^n$ 에 대해 $I_n v = v$ 이며 $A,\,B\in \text{GL}(n)$ 에 대해 $A(Bv)= (AB)v$ 이므로 $A\in \text{GL}(n)$ 은 $GL(n)$ 의 $\R^n$ 에 대한 작용이다. 
...
::: {#exm-Group_group_action_on_cosets}

#### 잉여류에의 작용

군 $G$ 와 $H\le G$ 에 $\pi_g (aH)=gaH$ 는 $G$ 에서 $H$ 에 대한 왼쪽 잉여류로의 작용이다.
...
### 안정부분군과 궤도 {#sec-Group_stabilizer_and_orbit}
::: {.callout-note icon="false"}

#### **안정 부분군, 궤도, 고정점**

::: {#def-Group_stabilizer_subgroup}

군 $G$ 가 집합 $S$ 에 대한 작용일 때 정해진 $s\in S$ 에 대해 

$$
G_s = \{g\in G : \pi_g (s)=s\}
$$ {#eq-Group_definition_of_stabilizer_subgroup}

는 $G$ 의 부분군이며 이를 $s$ 에 대한 **안정부분군 (stabilizer subgroup)** 이라고 한다.

$s\in S$ 에 대해 

$$
\text{Orb}_G(s) := \{\pi_g(s) : g\in G\}
$$ {#eq-Group_definition_of_orbit}

를 $s$ 의 **궤도 (orbit)** 이라고 한다. 또한

$$
\text{Fix}_G(\pi) = \{s\in S: \forall g\in G\, \pi_g(s) = s\}
$$ {#eq-Grou_defintion_of_fixed_point}

를 **고정점 (fixed point)*** 라고 한다.
:::
:::

</br><div class="border" style="background-color:#FFF0F5 ;padding:5px;">

::: {#prp-Group_stabilizer_subgroup_1}

@eq-Group_definition_of_stabilizer_subgroup 의 $G_s$ 는 $G$ 의 부분군이다.
...
$x,\,y\in G_s$ 이면 $xs=ys=s$ 이다. $ys=s$ 에 $y^{-1}$ 을 작용하면 $y^{-1}s=s$ 이다. 따라서 $xy^{-1}s=s$ 이므로 $G_s \le G$ 이다. $\square$

:::

</br><div class="border" style="background-color:#FFF0F5 ;padding:5px;">

::: {#prp-Group_stabilizer_subgroup_2}

군 $G$ 가 $S$ 에 대한 작용이라고 하자. $s,\,s'\in S,\, y\in G$ 에 대해 $s'=ys$ 이면 $G_{s'}=yG_sy^{-1}$ 이다.
...
$$
\begin{aligned}
x\in G_{s'} & \iff xs'=s' \iff xys = ys \iff y^{-1}xys = s \iff y^{-1}xy \in G_s \\[0.3em]
&\iff x\in yG_sy^{-1}.\qquad \square
\end{aligned}
$$ 
...
::: {#prp-Group_stabilizer_subgroup_3}

$\pi$ 가 $S$ 에 대한 $G$ 의 작용이라고 할 때 
...
::: {#thm-Group_orbit_stabilizer_theorem}

#### 궤도-안정부분군 정리

유한군 $G$ 가 유한집합 $S$ 에 작용하며 $s\in S$ 일 때 $|G| = |\text{Orb}_G(s)||G_s|$ 이다.
...
$G_S\le G$ 이므로 $G_s$ 에 의한 왼쪽 잉여류의 집합 $Y$ 를 정의 할 수 있다. 이제 $\Phi: Y\to \text{Orb}_G(s)$ 를 다음과 같이 정의하자.
...
우리는 @exm-Group_self_group_action 로부터 conjugation 이 군 $G$ 의 자신에 대한 작용임을 보았다. $g\in G$ 에 대한 conjugate 작용의 궤도는 $g$ 와 동등한 켤레류 이므로 다음의 결론을 얻는다.

<div class="border" style="background-color:#FFF0F5 ;padding:5px;">

::: {#cor-Group_orbit_stabilizer_theorem}

$K$ 가 유한군 $G$ 의 켤레류이면 $|K|{\big |} |G|$ 이다.
