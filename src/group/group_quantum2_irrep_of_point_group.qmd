...
여기서 사용할 구면조화 함수(Spherical harmonic function) $Y_l^m( \theta,\, \varphi)$ 는 르장드르 연관 함수(associated Legendre function) $P_l^m$ 과 음이 아닌 정수 $l$ 그리고 $|m|\le l$ 인 정수 $m$ 에 대해 아래와 같이 정의된다. (구면조화함수에 대해 자세한 것은 @arfken2013mathematical 을 참고하라.)
...
구면조화함수에서 $\theta,\, \varphi$ 를 정하는 기준이 되는 축(이를 polar axis 라고 하며 보통 $z$ 축으로 정한다.) 을 회전연산 $R$ 을 통해 바꾸어 $\theta',\, \varphi'$ 이 되었다고 하자. 이 때 다음이 성랍한다는 것이 알려져 있다.
...
&=\dfrac{1}{a} \left[1 + \dfrac{x}{a} + \dfrac{3x^2-r^2}{2a^2} + \dfrac{5x^3-3xr^2}{2a^3} + \dfrac{3r^4-10x^2r^2 + 35x^4}{8a^4} + \cdots \right] \\[0.3em]
\dfrac{1}{\sqrt{(x+a)^2+y^2+z^2}} &= \dfrac{1}{a}\left[1 - \dfrac{x}{a} + \dfrac{3x^2-r^2}{2a^2} - \dfrac{5x^3-3xr^2}{2a^3} + \dfrac{3r^4-10x^2r^2 + 35x^4}{8a^4} + \cdots\right] 
\end{aligned}
$$
...
$$
\begin{aligned}
V_O(\bf{r}) &= \dfrac{q}{4\pi\epsilon_0} \left[ \dfrac{6}{a} + \left( \sum_{i=1}^3 \dfrac{3x_i^2-r^2}{a_i^3}  + \dfrac{3r^4-10x_i^2r^2 + 35x_i^4}{8a_i^4}  \right)+\cdots\right]
\end{aligned}
$$
