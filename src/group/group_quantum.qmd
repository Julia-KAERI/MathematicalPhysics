...
$|\Psi_n\rangle$ 이 슈뢰딩거 방정식의 고유상태로 $E_n$ 의 고유값을 갖는다고 하자. $S$ 가 슈뢰딩거 방정식군에 포함된다면

$$
\hat{H}\hat{S}|\Psi_n \rangle = \hat{S}\hat{H} |\Psi_n \rangle = E_n \hat{S}|\Psi_n\rangle
$$

이므로 $\hat{S}|\Psi_n\rangle$ 역시 $E_n$ 의 고유값을 갖는 슈뢰딩거 방정식의 고유상태이다. 즉 어떤 대칭성이 있을 때 degeneracy 가 발생한다. 
...
우선 해밀토니안 $\hat{H}$ 의 고유값 $E_n$ 에 대해 $d$-fold normal degeneracy 가 존재한다고 하자. 이 $d$ 차원 고유공간을 $\mathcal{H}_n$ 이라고 하자. 그렇다면 $d$ 개의 정규직교 고유상태 $\{|\Gamma_n1\rangle,\ldots,|\Gamma_n d\rangle\}$ 가 존재한다. 여기서 $\Gamma_n$ 은 $\mathcal{H}_n$ 에 속한다는 것을 의미하며 그 뒤의 인덱스는 $\mathcal{H}_n$ 내의 고유상태의 순서를 의미한다. $\{|\Gamma_n 1\rangle,\ldots,|\Gamma_n d\rangle\}$ 에 의해 정의되는 벡터공간은 힐베르트 공간의 부분공간이며 $G_S$ 군에 속하는 모든 변환에 대한 불변 공간이다. 임의의 $S\in G_S$ 에 대해

$$
\hat{S}|\Gamma_n i\rangle = \sum_{j=1}^d D^{(\Gamma_n)}(S)_{ji} |\Gamma_n j\rangle
$$ {#eq-Group_transformation_by_schroedinger_equation_group}

인 $d\times d$ 행렬 $D^{(\Gamma_n)}(S)$ 가 존재한다.
...
$$
\begin{aligned}
\hat{R}\hat{S}|\Gamma_n i\rangle &= \hat{R}\sum_j D(S)_{ji}|\Gamma_nj\rangle = \sum_k\sum_j D(S)_{ji}D(R)_{kj}|\Gamma_n k\rangle \\[0.3em] 
&= \sum_k\left[D(R)D(S)\right]_{ki}|\Gamma_nk\rangle
\end{aligned}
$$

이다. 즉 $D(ST) = D(S)D(T)$ 이므로 $D$ 은 $G_S$ 에 대한 $d$ 차원 표현이다. 

이제 $D$ 이 기약표현이 아님을 가정하자. $D$ 이 모든 $G_S$ 에 대해 블럭대각행렬이라면 어떤 $|\Gamma_ni\rangle,\, |\Gamma_nj\rangle$ 이 존재하여 모든 $R\in G_S$ 에 대해 $\hat{R}|\Gamma_ni\rangle \ne |\Gamma_nj\rangle$ 이어야 한다. 그렇다면 여기에 accidental degeneracy 가 있는 것이며 normal degeneracy 라는 가정에 모순된다. 
...
이므로 $\left(D(R)\right)^{-1}=D(R)^\dagger$ 이다. 즉 $D=D^{(\Gamma_n)}$ 은 $d$ 차원 유니타리 기약표현이다. $\square$
...
$D$ 가 $G_S$ 에 대한 $d$ 차원 기약표현이라고 하자. 모든 $R\in G_S$ 에 대해 $\hat{R}\hat{T}=\hat{T}\hat{R}$ 이며 따라서 $\hat{H}D(T)= D(T)\hat{H}$ 이다. 슈어 보조정리(@lem-Group_schur_lemma) 에 의해 $\hat{H}$ 는 항등행렬의 상수배이므로 해밀토니안에는 $d$-fold degeneracy 가 존재한다. $\square$
...
슈뢰딩거 방정식의 기저벡터를 $|E_n,\, j\rangle$ 로 기술하는 경우를 생각하자. 여기서 $E_n$ 은 해밀토니안 연산자의 고유값이며 $j$ 는 이 고유값에 의한 고유공간의 $j$ 번째 기저벡터라는 의미이다. 여기서 @thm-Group_uniqueness_of_representation_of_schroedinger_group 는 고유값에 대해 슈뢰딩거 방정식 군의 특정 기약표현이 1-1 대응한다는 것을 말한다. 따라서 $E_n$ 에 해당하는 기약표현 $\Gamma_n$ 에 대해 $|\Gamma_nj\rangle$ 로 쓸 수 있으며 $E_n$ 에 의한 고유공간의 차원은 $\Gamma_n$ 기약표현의 차원이다.
...
군 $G$ 의 두 부분군 $H_1,\,H_2$ 에 대해

$$
\forall h_1\in H_1,\, h_2\in H_2,\, h_1h_2 = h_2h_1
$$

일 때 아래와 같이 정의되는 $H_1 \otimes  H_2$ 를 **direct product group** 이라고 한다.

$$
H_1 \otimes H_2 := \{h_1h_2: h_1\in H_1,\, h_2\in H_2\}
$$
...
@def-Group_direct_product_group 의 조건에서 $H_1 \otimes H_2$ 는 $G$ 의 부분군이다.
...
@prp-Group_how_to_be_subgroup 의 ($c$) 조건을 통해 증명한다.  $a,\,b\in H_1,\, x,\,y \in H_2$ 에 대해 

$$
(ax)^{-1}(by)= x^{-1}a^{-1}by = a^{-1}bx^{-1}y \in H_1\times H_2
$$ 

이므로 $H_1 \otimes H_2 \le G$ 이다. $\square$

:::

</br>

[직합표현과 직곱표현](group_representation1.qmd#sec-Group_direct_sum_representation_and_direct_product_representation) 을 참고하라. 군 $G$ 의 두 부분군 $H_1,\,H_2$ 에 대한 direct product group $H_1\times H_2$ 가 존재한다고 하자. 그리고 $H_1$ 에 대한 표현 $\Gamma_1$ 과 $H_2$ 에 대한 표현 $\Gamma_2$ 에 대해 $D^{(\Gamma_1 \otimes \Gamma_2)}$ 를 $D^{(\Gamma_1)}$ 과 $D^{(\Gamma_2)}$ 의 직곱표현으로 정의하자. 즉, $x\in H_1,\, y\in H_2$ 에 대해

$$
D^{(\Gamma_1 \otimes \Gamma_2)} (xy):= D^{(\Gamma_1)}(x)\otimes D^{(\Gamma_2)}(y)
...
@eq-Group_representation_of_direct_product_group 은 $H_1 \otimes H_2$ 의 표현이다.
...
::: {.proof}
@exr-Group_proof_of_direct_product_mutiplication 를 사용한다. $a,\,b\in H_1,\, x,\,y\in H_2$ 에 대해

$$
\begin{aligned}
D^{(\Gamma_1 \otimes \Gamma_2)}(ax)D^{(\Gamma_1 \otimes \Gamma_2)}(by)  &= [D^{(\Gamma_1)}(a)\otimes D^{(\Gamma_2)}(x)][D^{(\Gamma_1)}(b)\otimes D^{(\Gamma_2)}(y)] \\[0.3em]
&=D^{(\Gamma_1)}(ab)\otimes D^{(\Gamma_2)}(xy) \\[0.3em]
&=D^{(\Gamma_1 \otimes \Gamma_2)}(abxy) =D^{(\Gamma_1 \otimes \Gamma_2)}(axby) \qquad \qquad \square
\end{aligned}
$$
...
::: {#prp-Group_direct_product_representation_irreducibility}

@eq-Group_representation_of_direct_product_group 의 $\Gamma_1$ 과 $\Gamma_2$ 가 각각 $H_1,\,H_2$ 에 대한 기약표현이라면 $\Gamma_1 \otimes \Gamma_2$ 는 $H_1\otimes H_2$ 의 기약표현이다.
...
Wigner 책 16장 에 나와 있다고 하더라.(@tinkham2012group)
:::