...
이다. 여기서 $C$ 를 상수가 아닌 $x$ 에 대한 함수로 놓고 특이해를 구할 수 있는 경우가 있다. 즉 $C\to C(x)$ 로 변환하고 $y'$ 을 구하면

$$
y' = \left(\int^x p(x')\,dx'\right)\left(-p(x)C(x) + C'(x)\right) = -p(x)y(x) + C'(x) \left(\int^x p(x')\,dx'\right)
$$

이며 따라서

$$
C'(x)\left(\int^x p(x')\,dx'\right) = q(x)
$$
